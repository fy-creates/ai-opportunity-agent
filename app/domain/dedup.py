...
def opportunity_fingerprint(*, source: str, source_id: str | None, title: str, organization: str, url: str) -> str:
    identity = source_id.strip() if source_id else canonicalize_url(url)
    raw = "|".join((source.strip().lower(), identity.lower(), title.strip().lower(), organization.strip().lower()))
